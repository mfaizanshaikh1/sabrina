 ...
 * @access public
 */
class ButterBean_Review_Setting {
	...
	public function get_value() {
		...
		return ! $value && butterbeanReview()->is_new_post ? $this->default : $value;
	}
	...
	public function save() {
		...
		if ( ! $new_value && $old_value ) {
			if ( $this->name == 'review_car' ) delete_post_meta( $old_value, 'has_review_car' );
			delete_post_meta( $this->manager->post_id, $this->name );
		}

		// If the new value doesn't match the old value, set it.
		else if ( $new_value !== $old_value ) {
			if ( $this->name == 'review_car' ) {
				delete_post_meta( $old_value, 'has_review_car' );
				update_post_meta( $new_value, 'has_review_car', $this->manager->post_id );
			}
