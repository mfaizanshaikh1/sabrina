...
if (!function_exists('stm_custom_register')) {
    ...
    function stm_custom_register()
    {
        ...
        if (empty($errors)) {
            ...
            if( !is_wp_error($user_id ) && get_theme_mod('enable_email_confirmation', false) ) {

                $user_data['user_phone'] = $user_phone;

				stm_handle_premoderation($user_id, $user_data);

				$response['message'] = esc_html__('Please confirm your email', 'motors');
...
if (!function_exists('stm_ajax_add_a_car_media')) {
    ...
    function stm_ajax_add_a_car_media()
    {
        ...
        if(apply_filters('stm_listings_notify_updated', true)) {
	        ...
	        if(get_theme_mod('send_email_to_user', false) && !$updating && get_post_status($post_id) == 'pending') {
...
{
	return get_theme_mod( 'default_sort_by', 'date_high' );
}
