...
class Subscriptio_PayPal_EC_Gateway extends WC_Payment_Gateway
{
    ...
    public function __construct($id = null)
    {
        ...
        $this->method_title          = __('PayPal Express Checkout by RightPress', 'subscriptio-paypal-ec');
    ...
    public function get_notices()
    {
        ...
        if ($this->sandbox == 'no' && empty($this->api_username) || empty($this->api_password) || empty($this->api_signature)) {
            $notices['secret'] = __('PayPal Express Checkout payment gateway requires all API credentials to be set.', 'subscriptio-paypal-ec');
        }
    ...
    {
        wp_enqueue_script('subscriptio_paypal_ec_backend', SUBSCRIPTIO_PLUGIN_URL . '/assets/gateways/paypal-ec/js/backend.js', array('jquery'), SUBSCRIPTIO_VERSION);
    }
    ...
    public function set_express_checkout($order, $return_url, $cancel_url)
    {
        ...
        if (Subscriptio_Order_Handler::contains_subscription(RightPress_WC_Legacy::order_get_id($order))) {
    ...
    public function do_express_checkout_payment()
    {
        ...
        // Check everything
        if (empty($token) || (empty($payer_id) && RightPress_WC_Legacy::order_get_total($order) != 0) || empty($order_id)) {
        ...
        if (RightPress_WC_Legacy::order_get_total($order) == 0) {
        ...
        $do_ec_request = array(
            ...
            'AMT'           => RightPress_WC_Legacy::order_get_total($order),
            'PAYMENTACTION' => 'Sale',
            'CURRENCYCODE'  => strtoupper(RightPress_WC_Legacy::order_get_currency($order)),
        );
        ...
        // Save payment method
        RightPress_WC_Legacy::order_update_meta_data($order, '_payment_method', $this->id);
        RightPress_WC_Legacy::order_update_meta_data($order, '_payment_method_title', $this->title);
	...
	$nvp_response = $this->send_curl_request($nvp_request);
        ...
        if (in_array($result_message, array('Success', 'SuccessWithWarning'))) {
            ...
            // Save transaction id
            RightPress_WC_Legacy::order_update_meta_data($order, '_subscriptio_paypal_ec_transaction_id', $transaction_id);

            // Add billing agreement id to user/post meta
            if (isset($nvp_response_array['BILLINGAGREEMENTID'])) {
                $user_id = get_current_user_id();
                RightPress_WC_Legacy::order_update_meta_data($order, '_subscriptio_paypal_ec_billing_agreement', $nvp_response_array['BILLINGAGREEMENTID']);
                RightPress_WC_Legacy::customer_update_meta_data($user_id, '_subscriptio_paypal_ec_billing_agreement', $nvp_response_array['BILLINGAGREEMENTID']);
            }

            // Save token
            RightPress_WC_Legacy::order_update_meta_data($order, '_subscriptio_paypal_ec_token', $token);
    ...
    public function handle_trial($order, $token)
    {
        ...
        $this->log_add(__('Starting process of handling trial subscription from order #', 'subscriptio-paypal-ec') . RightPress_WC_Legacy::order_get_id($order));
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        // Get transaction id
        $transaction_id = RightPress_WC_Legacy::order_get_meta($order, '_subscriptio_paypal_ec_transaction_id', true);
