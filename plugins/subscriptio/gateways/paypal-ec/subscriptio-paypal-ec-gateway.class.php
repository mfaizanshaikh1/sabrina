...
class Subscriptio_PayPal_EC_Gateway extends WC_Payment_Gateway
{
    ...
    public function get_notices()
    {
        ...
        if ($this->sandbox == 'no' && empty($this->api_username) || empty($this->api_password) || empty($this->api_signature)) {
            $notices['secret'] = __('PayPal Express Checkout requires all API credentials to be set.', 'subscriptio-paypal-ec');
        }
    ...
    public function process_payment($order_id)
    {
        ...
        if (!$order) {
            ...
            wc_add_notice(__('Order not found. We have not charged you for this order. Please try again.', 'subscriptio-paypal-ec'), 'error');
            return;
        }
        ...
        // Check cart
            ...
            wc_add_notice(__('The cart is empty - we have not charged you for this order.', 'subscriptio-paypal-ec'), 'error');
            return;
        }
    ...
    public function do_express_checkout_payment()
    {
        ...
        if (empty($token) || (empty($payer_id) && $order->get_total() != 0) || empty($order_id)) {
            ...
            // Add error
            wc_add_notice(__('Unfortunately, your session for this payment has expired. Please try again.', 'subscriptio-paypal-ec'), 'error');
        ...
        // Prevent double payments
            ...
            // Add error
            wc_add_notice(__('Operation cancelled - current token was already used to make a payment.', 'subscriptio-paypal-ec'), 'error');
    ...
    public function handle_trial($order, $token)
    {
        ...
        $this->log_add(__('Starting process of handling trial subscription from order #', 'subscriptio-paypal-ec') . $order->get_id());
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        if ($result_message == 'Success' || $result_message == 'SuccessWithWarning') {
            $order->add_order_note(sprintf(__('%s of PayPal charge %s refunded.', 'subscriptio-paypal-ec'), wc_price($amount), $transaction_id));
