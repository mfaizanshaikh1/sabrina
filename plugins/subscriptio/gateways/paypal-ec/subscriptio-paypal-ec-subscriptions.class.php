...
class Subscriptio_PayPal_EC_Subscriptions
{
    ...
    public static function get_billing_agreement_id($order = null, $subscription = null)
    {
        ...
        if (!is_null($subscription)) {
            ...
            $billing_agreement_user = $customer->get_meta('_subscriptio_paypal_ec_billing_agreement', true);

            $billing_agreement_subscription = $subscription->get_payment_gateway_option('_subscriptio_paypal_ec_billing_agreement');
            ...
            $billing_agreement_order = $order->get_meta('_subscriptio_paypal_ec_billing_agreement', true);

            $customer = new WC_Customer($order->get_customer_id());
            $billing_agreement_order_user = $customer->get_meta('_subscriptio_paypal_ec_billing_agreement', true);
    ...
    public function save_billing_agreement_id($order_id)
    {
        ...
        $subscriptions = subscriptio_get_subscriptions_related_to_order($order_id);

        // Iterate over subscriptions
        foreach ($subscriptions as $subscription) {

            $customer = new WC_Customer($subscription->get_customer_id());
            $billing_agreement_id = $customer->get_meta('_subscriptio_paypal_ec_billing_agreement', true);

            $post_meta_billing_agreement_id = $subscription->get_payment_gateway_option('_subscriptio_paypal_ec_billing_agreement');

            if (empty($post_meta_billing_agreement_id)) {
                $subscription->add_payment_gateway_option('_subscriptio_paypal_ec_billing_agreement', $billing_agreement_id);
            }
        }
    }
    ...
    public static function do_reference_transaction($billing_agreement_id, $order)
    {
        ...
        $request = array(
            ...
            'CUSTOM'        => maybe_serialize(array('order_id' => $order->get_id())),
        );
