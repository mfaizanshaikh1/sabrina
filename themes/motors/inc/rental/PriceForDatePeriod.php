...
class PriceForDatePeriod
{
	...
	public static function getTotalByDays( $price, $varId )
	{
		...
		if($orderCookieData['order_days'] === '0' && $orderCookieData['ceil_days'] === '0' ) return 0;
				...
				foreach ( $cart as $cart_item ) {
					...
					if ( $buy_type == 'options' ) {
						...
						$cart_items[$buy_type][] = array( 'id' => $id, 'variation_id' => $variation_id, 'quantity' => $cart_item['quantity'], 'name' => $post->get_title(), 'price' => $item['price'], //stm_getDefaultVariablePrice($id),
							'total' => self::getTotalByDays( $item['price'], stm_get_wpml_product_parent_id( $item['parent_id'] ) ), 'subname' => get_post_meta( $id, 'cars_info', true ), 'payment_method' => get_post_meta( $variation_id, '_stm_payment_method', true ), 'days' => $fields['order_days'], 'hours' => ( isset( $fields['order_hours'] ) ) ? $fields['order_hours'] : 0, 'ceil_days' => $fields['ceil_days'], 'oldData' => $cartOldData );
	...
	{
		$id = ( $varId != '' ) ? $varId : stm_get_wpml_product_parent_id( get_the_ID() );

		$result = ($id) ? self::$wpdbObj->get_results( "SELECT * FROM " . self::$wpdbObj->prefix . "rental_price_date WHERE `variation_id` = {$id} ORDER BY id ASC" ) : null;
	...
	public static function addPeriodIntoDB( $varId, $datePickup, $dateDrop, $price )
	{
		...
		if ( !is_null( self::checkEntry( $datePickup, $dateDrop, $varId ) ) ) {
			self::$wpdbObj->update( $table_name, array( 'variation_id' => $varId, 'starttime' => strtotime( $datePickup ), 'endtime' => strtotime( $dateDrop ), 'price' => $price ), array( 'variation_id' => $varId, 'starttime' => strtotime( $datePickup ), 'endtime' => strtotime( $dateDrop ), ), array( '%d', '%d', '%d', '%d' ), array( '%d', '%d', '%d' ) );
		} else {
			self::$wpdbObj->insert( $table_name, array( 'variation_id' => $varId, 'starttime' => strtotime( $datePickup ), 'endtime' => strtotime( $dateDrop ), 'price' => $price ), array( '%d', '%d', '%d', '%d' ) );
		}
	}
	...
	public static function priceForDateView()
	{
        ...
        <div class="rental-price-date-wrap">
            <ul class="rental-price-date-list">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="date-pickup"
                                               value="<?php echo date( 'Y/m/d H:i', $val->starttime ); ?>"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="date-drop"
                                               value="<?php echo date( 'Y/m/d H:i', $val->endtime ); ?>"
				...
				?>
                <li>
                    <button class="repeat-fields button-primary button-large" <?php echo esc_attr( $disabled ); ?>>Add
                    </button>
                </li>
            </ul>
