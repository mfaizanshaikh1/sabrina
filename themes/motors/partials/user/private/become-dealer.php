			...
			if(!empty($_GET['stm_payment'])) {
				...
				<script>
					jQuery(window).on('load', function(){
						window.location.href = "<?php echo stm_generatePayment(); ?>";
					})
				</script>
				<?php
			}
	...
	<?php } else { ?>
		...
		if(!$errors and empty($errors_message)) {
			...
			if ( $user_id == $user_current->ID ) {
				...
				$subject = generateSubjectView('request_for_a_dealer', $args);
				$body = generateTemplateView('request_for_a_dealer', $args);
