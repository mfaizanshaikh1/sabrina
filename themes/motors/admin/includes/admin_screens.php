...
function stm_register_startup_screen() {
	...
	if ( !defined('ENVATO_HOSTED_SITE') ) {
		...
		if(!empty(get_option('subscriptio_options', array()))) {
			/*SUbscriptio 3.0*/
			$stm_admin_submenu_page_creation_method( $theme_name_sanitized, esc_html__( 'Remove Subscriptio', 'motors' ), esc_html__( 'Remove Subscriptio', 'motors' ), 'manage_options', $theme_name_sanitized . '-remove-subscriptio', 'stm_theme_admin_remove_subscriptio_page' );
		}
		...
		if(stm_check_hacked_categories()) {
            ...
            $stm_admin_submenu_page_creation_method(
                ...
                esc_html__( 'Listing Category patch', 'motors' ),
                'manage_options',
                $theme_name_sanitized . '-category-patch',
                'stm_theme_admin_listing_category_patch_page'
            );
        }
	} else {
		/*Demo Import*/
...
/*Install Demo*/
function stm_theme_admin_remove_subscriptio_page() {
	stm_startup_templates('remove_subscriptio');
}

/*Listing category patch*/
function stm_theme_admin_listing_category_patch_page() {
	stm_startup_templates('listing_category_patch');
}
...
function stm_get_admin_tabs( $screen = 'welcome' ) {
	...
	$patched = true;//get_option('stm_tax_patched', '');
	?>
	<div class="clearfix">
	...
	if( !empty($creds['t']) && !empty($notice) ): ?>
		<div class="stm-admin-message"><strong>Theme Registration Error:</strong> <?php echo stm_do_lmth($notice); ?></div><br>
	<?php endif; ?>
...
function stm_check_hacked_categories() {
    $opt = get_option('stm_vehicle_listing_options');

    if(strpos(json_encode($opt), 'script')) {
        return true;
    } else {
        return false;
    }
