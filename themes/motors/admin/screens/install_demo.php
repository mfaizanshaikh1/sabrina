...
$demos = array(
	...
	'listing_six'      => array(
		'label' => esc_html__('LISTING SIX', 'motors'),
	),
);
...
<div class="wrap about-wrap stm-admin-wrap  stm-admin-demos-screen">
	...
	<?php if( !empty($auth_code) ):
		$current_demo = '';
		?>
		<?php if (!empty($current_demo = get_option('stm_motors_chosen_template'))): ?>
	...
	<?php else: ?>
        <div class="stm-admin-message">
			<?php
            $url = admin_url("admin.php?page=stm-admin");
            printf(__('Please enter your <a href="%s">Activation Token</a> before running the Motors.', 'motors'), $url); ?>
        </div>
	<?php endif; ?>
