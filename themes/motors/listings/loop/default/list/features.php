...
<div class="single-car-actions">
	<ul class="list-unstyled clearfix">
		...
		<?php if(!empty($stock_number) and !empty($show_stock) and $show_stock): ?>
			<li>
				<div class="stock-num heading-font"><span><?php esc_html_e('stock', 'motors'); ?># </span><?php echo esc_attr($stock_number); ?></div>
					...
					<div class="check-available-mi">
					     <a href=<?= the_permalink() ?>>See Details</a>
					</div>
		<?php endif; ?>
		...
		<?php if(!empty($show_test_drive) and $show_test_drive): ?>
			<li >
				<a href="#" class="car-action-unit stm-schedule <?php if(stm_is_aircrafts()) echo 'heading-font'; ?>" data-toggle="modal" data-target="#test-drive" onclick="stm_test_drive_car_title(<?php echo esc_js(get_the_ID()); ?>, '<?php echo esc_js(get_the_title(get_the_ID())) ?>')">
		...
		<?php if(!empty($show_compare) and $show_compare): ?>
			<li data-compare-id="<?php echo esc_attr(get_the_ID()); ?>">
				<a href="#" class="car-action-unit add-to-compare stm-added <?php if(stm_is_aircrafts()) echo 'heading-font'; ?>" style="display: none;" data-id="<?php echo esc_attr(get_the_ID()); ?>" data-action="remove">
				...
				<a href="#" class="car-action-unit add-to-compare <?php if(stm_is_aircrafts()) echo 'heading-font'; ?>" data-id="<?php echo esc_attr(get_the_ID()); ?>" data-action="add">
			...
			<?php if(!empty($car_brochure)): ?>
				<li>
					<a
						...
						class="car-action-unit stm-brochure <?php if(stm_is_aircrafts()) echo 'heading-font'; ?>"
						title="<?php esc_html_e('Download brochure', 'motors'); ?>"
						download>
						<i class="stm-icon-brochure"></i>
						<?php (stm_is_aircrafts()) ? esc_html_e('Pdf brochure', 'motors') : esc_html_e('Car brochure', 'motors'); ?>
					</a>
				</li>
			<?php endif; ?>
		<?php endif; ?>
        ...
        <?php if(isset($reviewId) && !is_null($reviewId)): ?>
