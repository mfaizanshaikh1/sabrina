			...
			<div class="stm-login-register-form">
				<div class="stm-register-form">
					<form id="add-car-register-form" method="post">
						...
						<div class="row form-group">
							<div class="col-md-6">
								<h4><?php esc_html_e('Login ID *', 'motors'); ?></h4>
								<input class="user_validated_field" type="text" name="stm_nickname" placeholder="<?php esc_attr_e('Enter Login ID', 'motors') ?>"/>
							</div>
							<div class="col-md-6">
							...
							<label>
								...
								<?php esc_html_e('I accept the terms of the', 'motors'); ?>
									<?php if(!empty($link) and !empty($link['url'])): ?>
										<a href="<?php echo esc_url($link['url']); ?>" target="<?php echo esc_attr($link['target']); ?>"><?php esc_html_e($link['title'], 'motors') ?></a>
									<?php endif; ?>
							</span>
							</label>
						</div>
