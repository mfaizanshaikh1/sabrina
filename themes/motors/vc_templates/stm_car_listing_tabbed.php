...
$tab_unique_found = 'found-cars-' . rand( 0, 99999 );
?>
<div class="car-listing-tabs-unit <?php echo esc_attr( $tab_unique ); ?>">
        ...
        <div class="stm-listing-tabs">
            <ul class="heading-font" role="tablist">
                    ...
                    <?php foreach ( $filter_cats as $filter_cat ): $filter_cats_counter++; ?>
                        <?php if ( !empty( $filter_cat[0] ) and !( empty( $filter_cat[1] ) ) ): ?>
                            ...
                            <?php if ( !empty( $current_category ) ): ?>
                                <li <?php if ( $filter_cats_counter == 1 ) {
                ...
                <?php if ( !empty( $enable_search ) and $enable_search ): ?>
                    <li <?php if ( $filter_cats_counter == 0 ) {
                        echo esc_attr( 'class=active' );
                    } ?>>
    ...
    <div class="car-listing-main-part">
        <div class="tab-content">
            <?php if ( !empty( $filter_cats ) ): ?>
                <?php foreach ( $filter_cats as $filter_cat ): $filter_cats_counter++; ?>
                    <?php if ( !empty( $filter_cat[0] ) and !( empty( $filter_cat[1] ) ) ): ?>
                        ...
                        ?>
                        <div role="tabpanel" class="tab-pane <?php if ( $filter_cats_counter == 1 ) {
            ...
            <!--Search tab-->
            <?php if ( !empty( $enable_search ) and $enable_search ): ?>
                <div role="tabpanel" class="tab-pane <?php if ( $filter_cats_counter == 0 ) {
                    echo esc_attr( 'active' );
                } ?>" id="car-listing-tab-search">
                    <div class="found-cars-clone">
                        <div class="found-cars heading-font"><i
                                    class="stm-icon-car"></i><?php esc_html_e( 'available', 'motors' ); ?> <span
                                    class="blue-lt"><?php echo esc_attr( $all_cars->found_posts ); ?> <?php esc_html_e( ' cars', 'motors' ); ?></span>
                        </div>
                    </div>
                                        ...
                                        <?php if ( !empty( $taxonomy_term[0] ) ): ?>
                                            <div class="col-md-<?php echo esc_attr( $filter_columns_number ); ?> col-sm-6">
                                                <div class="form-group">
                                                    <select name="<?php echo ( esc_attr( $taxonomy_term_key ) == "price" ) ? "max_price" : esc_attr( $taxonomy_term_key ) ?>"
                                                            class="form-control">
                                        ...
                                        <div class="row">
                                            <div class="col-md-8 col-sm-12">
                                                <button type="submit" class="button icon-button"><i
                                                            class="stm-icon-search"></i><?php esc_html_e( 'Search', 'motors' ); ?>
                                                </button>
                                            </div>
                                            <div class="col-md-4 hidden-sm hidden-xs">
                                                <a href="" class="reset-all reset-styled"
                                                   title="<?php esc_html_e( 'Reset search fields', 'motors' ); ?>"><i
