...
if (empty($_COOKIE['compare_ids'])) {
    ...
    foreach ($_COOKIE['compare_ids'] as $key => $val) {
        if (get_post_status($key) == 'publish') {
            $_COOKIE['compare_ids'][$key] = $val;
            $compare_ids[$key] = $val;
        } else {
            ?>
            <script>
                (function ($) {
                    $(document).ready( function () {
                        stm_remove_cookie(<?php echo stm_do_lmth($key); ?>);
                    });
                })(jQuery);
            </script>
            <?php
...
$add_to_text = (stm_is_aircrafts()) ? esc_html__('Add to compare', 'motors') : esc_html__('Add car to compare', 'motors');
...
$compareEmptyPLCHLDR = (stm_is_aircrafts()) ? 'ac_empty_compare.jpg' : 'compare-empty.jpg';
?>
                                ...
                                <div class="image">
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <img class="stm-compare-empty"
                                             src="<?php echo esc_url(get_stylesheet_directory_uri() . '/assets/images/' . $compareEmptyPLCHLDR); ?>"
