...
stm_motors_enqueue_scripts_styles('stm_inventory_on_map');
...
foreach ($cars->get_posts() as $k => $val) {
    ...
    if (get_post_meta($val->ID, "stm_lat_car_admin") != null && $val->post_status == "publish" || $val->post_status == "private") {
        ...
        $termOne = wp_get_post_terms($val->ID, $carsInfo[0]["key"]);
        $termTwo = wp_get_post_terms($val->ID, $carsInfo[1]["key"]);
        $termThree = wp_get_post_terms($val->ID, $carsInfo[2]["key"]);

        if (isset($carsInfo[0]) && !empty($termOne) || isset($carMeta[$carsInfo[0]["key"]][0])) {
            $mileage = (isset($carMeta[$carsInfo[0]["key"]][0])) ? $carMeta[$carsInfo[0]["key"]][0] : $termOne[0]->name;
        }

        if (isset($carsInfo[1]) && !empty($termTwo) || isset($carMeta[$carsInfo[1]["key"]][0])) {
            $engine = (isset($carMeta[$carsInfo[1]["key"]][0])) ? $carMeta[$carsInfo[1]["key"]][0] : $termTwo[0]->name;
        }

        if (isset($carsInfo[2]) && !empty($termThree) || isset($carMeta[$carsInfo[2]["key"]][0])) {
            $transmission = (isset($carMeta[$carsInfo[2]["key"]][0])) ? $carMeta[$carsInfo[2]["key"]][0] : $termThree[0]->name;
        }
        ...
        $carsData[$i]["mileage"] = $mileage;
        $carsData[$i]["engine"] = $engine;
        $carsData[$i]["transmission"] = $transmission;
...
?>
    ...
    <div class="stm-inventory-map-wrap">
        <div<?php echo(($map_style) ? ' style="' . esc_attr(implode(' ', $map_style)) . ' margin: 0 auto; "' : ''); ?>
                id="stm_map-<?php echo esc_attr($id); ?>" class="stm_gmap"></div>
        <div class="stm-inventory-map-filter-arrow-wrap">
            ...
            <div class="stm-inventory-map-filter-wrap">
                    ...
                    <form action="<?php echo stm_listings_current_url() ?>" method="get" data-trigger="filter-map">
                        <div class="filter filter-sidebar ajax-filter">
                            ...
                            <input type="hidden" name="popular"
                                   value="<?php echo esc_attr(stm_listings_input('popular')); ?>"/>
                ...
                <div class="stm-inventory-map-btn">
                    <div class="stm-inventory-map-cars-count"
                         data-sprint="<?php echo esc_html__("%s matches", "motors") ?>"><?php echo sprintf(esc_html__("%s matches", "motors"), count($carsData)); ?></div>
        ...
        jQuery(document).ready(function ($) {
            ...
            $(".stm-inventory-map-btn input[type='submit']").on("click", function () {
                ...
                $.ajax({
                    ...
                    type: "GET",
                    data: 'action=stm_ajax_get_cars_for_inventory_map&security=' + getCarsInvtMap + '&' + data.join('&'),
                    dataType: "json",
