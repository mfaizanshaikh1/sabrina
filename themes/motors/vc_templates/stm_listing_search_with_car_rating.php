...
stm_motors_enqueue_scripts_styles('stm_listing_search_with_car_rating');
...
if(!empty($taxonomy)) {
    ...
    unset($tax[count($tax) - 1]);

    //$tax = array_filter($tax);

    $taxonomy = implode(',', $tax);
}

$taxonomy = (!empty($taxonomy)) ? $taxonomy : 'make,serie,ca-year,price';
                ...
                <div class="filter">
                    <form id="listing-with-review" action="<?php echo esc_url(stm_get_listing_archive_link()); ?>" method="GET">
                        <div class="stm-filter-tab-selects filter stm-vc-ajax-filter">
                            <?php stm_listing_filter_get_selects($taxonomy, 'stm_all_listing_tab', $words, $show_amount); ?>
                        </div>
                        ...
                        <input type="hidden" name="filter-params" value="<?php echo esc_html($taxonomy); ?>">
